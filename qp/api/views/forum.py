...
class qpForumsView(APIView):

    def get(self, request):
        # ===---
        try:
            worlds = qpForum.objects.all()
        except Exception as e:
            print("Error on qpForumsView > get : ", e)
            return Response({"valid": False}, status=status.HTTP_400_BAD_REQUEST)
        # ===---
        worlds_data = []
        for world in worlds:
            world_data = qpForumView.get_world(request, world.pk)
...
class qpForumView(APIView):
    ...
    def get_world(request, pk, singleton):
        ...
        :param pk: (int) primary key of qpForum.
        ...
        # ===---
        try:
            world = qpForum.objects.get(
                pk=pk
            )
        except Exception as e:
            print("Error on qpForumView > get_world : ", e)
            return
        # ===---
        world_data = {
            "id": int(world.pk),
        ...
        if singleton in ["index"]:
            ...
            for zone in world.zones.all():
                zone_data = qpForumView.get_zone(request, zone.pk, singleton)
    ...
    def get_zone(request, pk, singleton):
        ...
        :param request: (any) django request.
        :param pk: (int) primary key of qpForumZone.
        :param singleton: (str) keyword where the data is showed ("index", "zone", etc.)
        ...
        # ===---
        try:
            zone = qpForumZone.objects.get(
                pk=pk
            )
        except Exception as e:
            print("Error on qpForumView > get_zone : ", e)
            return
        # ===---
        zone_data = {
            "id": zone.pk,
        ...
        if singleton in ["index", "zone"]:
            ...
            for territory in zone.territories.all():
                territory_data = qpForumView.get_territory(request, territory.pk, singleton)
    ...
    def get_territory(request, pk, singleton):
        ...
        :param request: (any) django request.
        :param pk: (int) primary key of qpForumTerritory.
        :param singleton: (str) keyword where the data is showed ("index", "zone", etc.)
        ...
        # ===---
        try:
            territory = qpForumTerritory.objects.get(
                pk=pk
            )
        except Exception as e:
            print("Error on qpForumView > get_territory : ", e)
            return
        # ===---
        territory_data = {
            "id": territory.pk,
        ...
        if singleton in ["territory"]:
            ...
            for sector in territory.sectors.all():
                sector_data = qpForumView.get_sector(request, sector.pk, singleton)
        ...
        if singleton in ["territory"]:
            ...
            for chapter in chapters:
                chapter_data = qpForumView.get_chapter(request, chapter.pk, singleton)
    ...
    def get_sector(request, pk, singleton):
        ...
        :param request: (any) django request.
        :param pk: (int) primary key of qpForumSector.
        :param singleton: (str) keyword where the data is showed ("index", "zone", etc.)
        ...
        # ===---
        try:
            sector = qpForumSector.objects.get(
                pk=pk
            )
        except Exception as e:
            print("Error on qpForumView > get_sector : ", e)
            return
        # ===---
        sector_data = {
            "id": sector.pk,
    ...
    def get_chapter(request, pk, singleton):
        ...
        :param request: (any) django request.
        :param pk: (int) primary key of qpForumChapter.
        :param singleton: (str) keyword where the data is showed ("index", "zone", etc.)
        ...
        # ===---
        try:
            chapter = qpForumChapter.objects.get(
                pk=pk
            )
        except Exception as e:
            print("Error on qpForumView > get_chapter : ", e)
            return
        # ===---
        chapter_data = {
            "id": chapter.pk,
        ...
        if singleton in ["chapter"]:
            ...
            for message in messages:
                message_data = qpForumView.get_message(request, message.pk, singleton)
    ...
    def get_message(request, pk, singleton):
        ...
        :param request: (any) django request.
        :param pk: (int) primary key of qpForumMessage.
        :param singleton: (str) keyword where the data is showed ("index", "zone", etc.)
        ...
        # ===---
        try:
            message = qpForumMessage.objects.get(pk=pk)
        except Exception as e:
            print("Error on qpForumView > get_message : ", e)
            return
        # ===---
        message_data = {
            "id": message.pk,
    ...
    def get(self, request, pk):
        # ===---
        world_data = qpForumView.get_world(request, pk, "index")
        # ===---
        return Response({
            "world": world_data,
            "valid": True
...
class qpForumZoneView(APIView):
    permission_classes = [AllowAny]

    def get(self, request, pk):
        # ===---
        zone_data = qpForumView.get_zone(request, pk, "zone")
        world_data = qpForumView.get_world(request, zone_data["world"], "zone")
        # ===---
        return Response({
            "world": world_data,
            "zone": zone_data,
...
class qpForumTerritoryView(APIView):
    permission_classes = [AllowAny]

    def get(self, request, pk):
        # ===---
        territory_data = qpForumView.get_territory(request, pk, "territory")
        zone_data = qpForumView.get_zone(request, territory_data["zone"], "territory")
        world_data = qpForumView.get_world(request, zone_data["world"], "territory")
        # ===---
        return Response({
            "world": world_data,
            "zone": zone_data,
...
class qpForumSectorView(APIView):
    permission_classes = [AllowAny]

    def get(self, request, pk):
        # ===---
        sector_data = qpForumView.get_sector(request, pk, "sector")
        territory_data = qpForumView.get_territory(request, sector_data["territory"], "sector")
        zone_data = qpForumView.get_zone(request, sector_data["zone"], "sector")
        world_data = qpForumView.get_world(request, sector_data["world"], "sector")
        # ===---
        return Response({
            "world": world_data,
            "zone": zone_data,
...
class qpForumChapterView(APIView):
    permission_classes = [AllowAny]

    def get(self, request, pk):
        # ===---
        chapter_data = qpForumView.get_chapter(request, pk, "chapter")
        sector_data = qpForumView.get_sector(request, chapter_data["sector"], "chapter")
        territory_data = qpForumView.get_territory(request, chapter_data["territory"], "chapter")
        zone_data = qpForumView.get_zone(request, chapter_data["zone"], "chapter")
        world_data = qpForumView.get_world(request, chapter_data["world"], "chapter")
        # ===---
        return Response({
            "world": world_data,
            "zone": zone_data,
